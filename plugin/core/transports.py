...
StateStrings = {
	...
	STATE_EOF				:	"STATE_EOF",
	STATE_EOF_IOERROR		:	"STATE_EOF_IOERROR",
	STATE_EOF_UNEXPECTED	:	"STATE_EOF_UNEXPECTED"
}
...
class StdioTransport(Transport):
	def __init__(self, process: 'subprocess.Popen', settings: Settings) -> None:
	...
	def read_stdout(self) -> None:
		...
		while running and self.process and state != STATE_EOF:
			...
			try:
				...
				state = STATE_EOF_IOERROR
				break
			except UnexpectedProcessExitError:
				self.close()
				debug("process became None")
				state = STATE_EOF_UNEXPECTED
				break
		debug(
			"process {} stdout ended {} ... {}[{}]".format(
				pid, "(still alive)" if self.process else "(terminated)",
				StateStrings[state], state
			))
		if self.process:
	...
	def write_stdin(self) -> None:
		while self.process:
			...
			if message is None:
				...
				try:
					msgbytes = bytes(message, self.settings.origin_encoding)
					try:
